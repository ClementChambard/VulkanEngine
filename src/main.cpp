...
bool checkExtensionsSupport(const char** exts, uint32_t cnt)
{
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        for (const auto& extension : extensions) {
            if (strcmp(exts[i], extension.extensionName) == 0) {
                extensionFound = true;
                break;
            }
        }
...
private:
    ...
    void createInstance()
    {
        ...
        if (!checkExtensionsSupport(extensions.data(), static_cast<uint32_t>(extensions.size()))) {
    ...
    void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& memory)
    {
    ...
    bool isDeviceSuitable(VkPhysicalDevice device)
    {
        ...
        if (extensionsSupported) {
            ...
            swapChainAdequate = !swapcChainSupport.formats.empty() && !swapcChainSupport.presentModes.empty();
        }
    ...
    void drawFrame()
    {
        vkWaitForFences(device, 1, &inFlightFences[currentImage], VK_TRUE, UINT64_MAX);

        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentImage], VK_NULL_HANDLE, &imageIndex);
        ...
        vkResetFences(device, 1, &inFlightFences[currentImage]);

        vkResetCommandBuffer(commandBuffers[currentImage], 0);
        recordCommandBuffer(commandBuffers[currentImage], imageIndex);
        ...
        if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentImage]) != VK_SUCCESS) {
